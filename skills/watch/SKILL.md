...
- `--whisper groq|openai|local` — force a specific Whisper backend (default: prefer Groq, then OpenAI, then local offline whisper-ctranslate2 if installed)
...
3. **Local Whisper fallback (offline, no key).** If no API key is set but `whisper-ctranslate2` is on PATH (`uv tool install whisper-ctranslate2` or pipx), the script transcribes the audio entirely on-device with faster-whisper. Model defaults to `medium`; override with `WATCH_LOCAL_WHISPER_MODEL` in `~/.config/watch/.env` (e.g. `base`, `small`, `large-v3`). First use downloads the model (~1.5 GB for medium). Force it with `--whisper local`.

Both keys live in `~/.config/watch/.env`. The script prefers Groq, then OpenAI, then local; override with `--whisper openai` or `--whisper local`. Use `--no-whisper` to skip the fallback entirely.
...
- **No transcript available** → captions missing AND no Whisper backend worked (no API key, no local whisper-ctranslate2 install, or the backend failed). Script prints a hint pointing to setup. Proceed frames-only and tell the user — or offer to install the offline fallback (`uv tool install whisper-ctranslate2`).
