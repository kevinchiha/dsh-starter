...
# Offline fallback: faster-whisper CLI (`uv tool install whisper-ctranslate2`).
# Used when no API key is set, or forced with --whisper local.
LOCAL_BIN = "whisper-ctranslate2"
LOCAL_MODEL_DEFAULT = "medium"
...
def load_api_key(preferred: str | None = None) -> tuple[str, str] | tuple[None, None]:
    """Return (backend, api_key). Prefers Groq, then OpenAI, then local whisper.

    If `preferred` is "groq", "openai", or "local", only that backend is
    considered. The local backend needs no key; its api_key slot is a dummy.
    """
    ...
    for key_name, backend in candidates:
        value = _from_env(key_name) or _dotenv_value(key_name)
        if value:
            return backend, value

    if preferred in (None, "local") and shutil.which(LOCAL_BIN):
        return "local", "local"
...
def _transcribe_local(audio_path: Path) -> list[dict]:
    """Transcribe offline with whisper-ctranslate2 (faster-whisper). No upload.

    Model defaults to "medium"; override with WATCH_LOCAL_WHISPER_MODEL in the
    environment or ~/.config/watch/.env (e.g. base, small, medium, large-v3).
    """
    out_dir = audio_path.parent / "local-whisper"
    out_dir.mkdir(parents=True, exist_ok=True)
    model = (
        os.environ.get("WATCH_LOCAL_WHISPER_MODEL")
        or _dotenv_value("WATCH_LOCAL_WHISPER_MODEL")
    ...
    result = subprocess.run(cmd, capture_output=True, text=True)
    if result.returncode != 0:
        raise SystemExit(f"local whisper failed: {result.stderr.strip()[-400:]}")
    json_path = out_dir / f"{audio_path.stem}.json"
    if not json_path.exists():
        raise SystemExit(f"local whisper produced no output at {json_path}")
...
def transcribe_video(
    ...
    if not backend or not api_key:
        ...
        raise SystemExit(
            ...
            "in the environment or in ~/.config/watch/.env, or install the offline "
            f"fallback with `uv tool install {LOCAL_BIN}` (or pipx). "
    ...
    if backend == "local":
        # No upload cap offline — always transcribe the whole file in one pass.
        print(
            f"[watch] audio: {audio_bytes / 1024:.0f} kB — transcribing locally ({LOCAL_BIN})…",
            file=sys.stderr,
        )
