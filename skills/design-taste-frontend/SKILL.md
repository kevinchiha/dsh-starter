...
description: "Builds the frontend code for landing pages, portfolios and redesigns with anti-slop guardrails: hard layout rules, a three-dial configuration, working GSAP skeletons (sticky-stack, horizontal-pan, scroll-reveal), and reduced-motion and performance limits. Use when writing the page, not when planning it. Take the palette and design system from ui-ux-pro-max first, then implement here."
---
...
2. **Real web images second.** When no gen tool is available, use real photography sources. Acceptable defaults:
   * **Generate the real asset first.** If an `imagegen` CLI is installed, generate the asset with
     `imagegen gen ... -o assets/<name>.png`; otherwise use `https://picsum.photos/seed/<word>/<w>/<h>`
     placeholders. An art-directed image beats stock filler,
     and costs about a cent. Fall back to a placeholder only for throwaway scaffolding.
...
* **NO broken Unsplash links.** If an `imagegen` CLI is installed, generate the asset with `imagegen gen ... -o assets/<name>.png`; otherwise use `https://picsum.photos/seed/<word>/<w>/<h>` placeholders, or actual assets.
